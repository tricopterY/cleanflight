...
#define MSP_ESC_SERVO_CONFIG            77 //out message         Returns FC ESC-SERVO parameters
#define MSP_SET_ESC_SERVO_CONFIG        78 //in message          Sets FC ESC-Throttle parameters
...
static bool processOutCommand(uint8_t cmdMSP)
{
    ...
    case MSP_FAILSAFE_CONFIG:
        headSerialReply(8);
        serialize8(currentProfile->failsafeConfig.failsafe_delay);
        serialize8(currentProfile->failsafeConfig.failsafe_off_delay);
        serialize16(currentProfile->failsafeConfig.failsafe_throttle);//also in MSP_MISC, just to keep things in sync for now
        serialize16(currentProfile->failsafeConfig.failsafe_min_usec);
        serialize16(currentProfile->failsafeConfig.failsafe_max_usec);                                
        break;
    case MSP_LOOP_TIME:
        headSerialReply(2);
...
static bool processInCommand(void)
{
    ...
    case MSP_SET_FAILSAFE_CONFIG:
        currentProfile->failsafeConfig.failsafe_delay = read8();
        currentProfile->failsafeConfig.failsafe_off_delay = read8();
        currentProfile->failsafeConfig.failsafe_throttle = read16();//also in MSP_SET_MISC, just to keep things in sync for now
        currentProfile->failsafeConfig.failsafe_min_usec = read16();
        currentProfile->failsafeConfig.failsafe_max_usec = read16();                
        break;
